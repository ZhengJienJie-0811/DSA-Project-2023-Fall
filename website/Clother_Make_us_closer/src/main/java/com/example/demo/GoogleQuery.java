	...
	public HashMap<String, String> query() throws IOException {
		...
		for (Element li : lis) {
			try {
				...
				// System.out.println(child_url);
				// String edit_child_url =
				// child_url.select("a").get(0).attr("href").replace("/url?q=", "");
				// System.out.println("This is edit_child URL");
				// System.out.println(edit_child_url);
				// String edit_child_url_2 =
				// child_url.select("a").get(1).attr("href").replace("/url?q=", "");
			...
			for (Element link : links) {
				...
				if (fifteen_childs.size() < 10) {
					...
					if (fifteen_childs.size() < 15) {
						...
						if (child_url.contentEquals("https://www.空中家教.com/web/") || child_url
								.contentEquals("https://help.voicetube.com/hc/zh-tw/categories/360001591732")) {
							continue;
						}
